...
<main>
    ...
    <section class="section-container my-5 pt-5 d-md-flex gap-5">
      <div class="single-product__img w-100" id="main-img">
        <img src="{{asset("front")}}/assets/images/{{$products->image}}" alt="">
      </div>
      <div class="single-product__details w-100 d-flex flex-column justify-content-between">
        <div>
          <h4>{{$products->name}}</h4>
          <div class="product__author">{{$products->author}}</div>
          <div class="product__author">{{$products->number_of_pages}}</div>
          ...
          <div class="d-flex w-100 gap-2 mb-3">
              <form method="post" action="{{route('cart',$products->id)}}">
                @csrf
                <div class="single-product__quanitity position-relative">
                  <input class="single-product__input text-center px-3" type="number" name="quantity" placeholder="---">
